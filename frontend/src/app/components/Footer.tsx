      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            ...
            <div key={title}>
              ...
              <ul className="space-y-2">
                {links.map(([label, href]) => (
                  <li key={label}>
                    <Link to={href} className="text-sm text-gray-500 transition-colors hover:text-[#FF385C] hover:underline">{label}</Link>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t border-gray-200 pt-8 md:flex-row">
          ...
          <div className="flex items-center gap-5">
            {["Privacy", "Terms", "Sitemap"].map(item => <button key={item} type="button" className="text-sm text-gray-500 transition-colors hover:text-gray-900">{item}</button>)}
