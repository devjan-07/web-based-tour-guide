...
import { Languages, MapPin, Search, ShieldCheck, Star, UserRound } from "lucide-react";
...
export default function GuidesPage() {
  ...
  return <div className="min-h-screen bg-[#f8fafc]">
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-7 flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wider text-rose-500">Available now</p><h2 className="mt-1 text-2xl font-black text-gray-900">Guides for your kind of trip</h2></div><Link to="/tourist/plan" className="hidden items-center gap-2 text-sm font-bold text-rose-500 sm:flex">Plan a trip</Link></div>
      {loading ? <div className="rounded-3xl bg-white p-12 text-center text-sm text-gray-400">Finding available guides...</div> : filtered.length ? <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filtered.map(guide=><article key={guide.id} className="overflow-hidden rounded-3xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
        ...
        <Link to="/tourist/plan" className="mt-4 flex items-center justify-center rounded-xl bg-[#FF385C] px-4 py-2.5 text-sm font-bold text-white">Use in trip planner</Link>
