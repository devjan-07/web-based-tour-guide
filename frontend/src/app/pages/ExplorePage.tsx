...
function StayGrid({ items }: { items: Accommodation[] }) {
  return <><SectionHeading title="Places to stay" text="Choose a comfortable base that fits the way you travel." />
    {items.length ? <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{items.map(item => <article key={item.id} className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-gray-200"><img src={item.image} alt={item.name} className="h-48 w-full object-cover" /><div className="p-4"><div className="flex justify-between gap-2"><span className="rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-bold text-gray-600">{item.type}</span><span className="text-xs font-bold text-gray-600">★ {Number(item.rating || 0).toFixed(1)}</span></div><h3 className="mt-3 font-extrabold text-gray-900">{item.name}</h3><p className="mt-1 text-xs text-gray-500">{item.location}, {item.country}</p><p className="mt-4 text-lg font-black text-gray-900">LKR {Number(item.price || 0).toLocaleString()} <span className="text-xs font-medium text-gray-400">/ night</span></p><Link to={`/tourist/accommodations/${item.id}/book`} className="mt-4 flex items-center justify-center rounded-xl bg-[#FF385C] px-4 py-2.5 text-sm font-bold text-white hover:opacity-90">View stay</Link></div></article>)}</div> : <EmptyState />}</>;
}
...
function VehicleGrid({ items }: { items: Vehicle[] }) {
  return <><SectionHeading title="Move around with ease" text="Browse available vehicles for self-drive or supported travel arrangements." />
    {items.length ? <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{items.map(item => <article key={item.id} className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-gray-200"><img src={item.image} alt={item.name} className="h-48 w-full object-cover" /><div className="p-4"><p className="text-xs font-bold uppercase tracking-wider text-orange-600">{item.type}</p><h3 className="mt-1 font-extrabold text-gray-900">{item.name}</h3><p className="mt-1 text-xs text-gray-500">{item.brand} {item.model} · {item.capacity} seats</p><p className="mt-4 text-lg font-black text-gray-900">LKR {Number(item.pricePerDay || 0).toLocaleString()} <span className="text-xs font-medium text-gray-400">/ day</span></p><Link to={`/tourist/vehicles/${item.id}/book`} className="mt-4 flex items-center justify-center rounded-xl bg-[#003580] px-4 py-2.5 text-sm font-bold text-white">View vehicle</Link></div></article>)}</div> : <EmptyState />}</>;
}
