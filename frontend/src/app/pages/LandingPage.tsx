        ...
        {isFiltering ? (
          ...
          <>
            ...
            <section className="px-4 pb-14 md:pb-16">
              <div className="mx-auto max-w-7xl">
                <SectionHeading eyebrow="How Voyara works" title="From inspiration to itinerary" description="A simple travel flow that keeps discovery, planning and booking connected." />
                <div className="mt-7 grid gap-4 md:grid-cols-3">
                  {[{ step: "01", title: "Discover", text: "Explore destinations, packages and local guides.", icon: "✦" }, { step: "02", title: "Plan", text: "Save ideas, compare packages and match trip resources to your needs.", icon: "↗" }, { step: "03", title: "Book", text: "Choose dates and create your booking when the plan is ready.", icon: "✓" }].map(({ step, title, text, icon }) => (
                    <div key={step} className="group rounded-3xl border border-gray-200 bg-white p-6 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-black tracking-[0.2em] text-rose-500">{step}</span>
                        <span className="flex h-9 w-9 items-center justify-center rounded-full bg-rose-50 text-sm font-black text-rose-500 transition group-hover:bg-rose-500 group-hover:text-white">{icon}</span>
                      </div>
                      <h3 className="mt-5 text-lg font-extrabold text-gray-900">{title}</h3>
            ...
            <section className="border-y border-gray-100 bg-gray-50 px-4 py-14 md:py-16">
              <div className="mx-auto max-w-7xl">
                <SectionHeading eyebrow="Why Voyara" title="One place for the important parts of your journey" description="Use the project's existing destination, package, guide, accommodation, vehicle and booking capabilities together instead of planning them separately." />
                <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  {[["Discover", "Find places and travel experiences from the same platform."], ["Compare", "Review package options side by side before choosing."], ["Personalise", "Match guides, stays and transport to your trip needs."], ["Manage", "Keep saved ideas and bookings connected in My Trip."]].map(([title, text]) => (
                    <div key={title} className="rounded-2xl bg-white p-5 ring-1 ring-gray-200">
